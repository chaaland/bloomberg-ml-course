...
def projected_batch_grad_desc_lasso(X, y, alpha=0.1, lambda_reg=1, num_iter=1000):
    ...
    for i in range(num_iter):
        ...
        theta_neg_grad_step = -theta_pos_grad_step

        # take gradient step
        theta_pos = theta_pos - alpha * theta_pos_grad_step
        theta_neg = theta_neg - alpha * theta_neg_grad_step
...
def projected_sgd_lasso(X, y, alpha=0.1, lambda_reg=1, num_iter=1000):
    """
    ...
    theta_pos = 0.1 * np.random.randn(num_features) 
    theta_neg = 0.1 * np.random.randn(num_features)
    loss_hist = zeros((num_iter, num_instances))
    theta_hist = zeros((num_iter, num_instances, num_features))
    ...
    for i in range(num_iter):
        ...
        for j, index in enumerate(indices):
            ...
            theta_neg_sgd_step = -theta_pos_sgd_step
            ...
            theta_hist[i,j,:] = (theta_pos - theta_neg)
            regularization_loss = np.linalg.norm(theta_hist[i,j,:], ord=1)
            data_loss = compute_square_loss(X, y, theta_hist[i,j,:])
            loss_hist[i,j] = data_loss + lambda_reg * regularization_loss
            cnt += 1

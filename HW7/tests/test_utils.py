...
def test_node_backward(node, init_vals, delta=1e-7):
    ...
    for parent in node.get_predecessors():
        ...
        while not it.finished:
            ...
            parent.out[it.multi_index] = parent_out[
                it.multi_index
            ]  # set it back to how it
            ...
            it.iternext()
        logging.debug(
            "(Node %s) Max rel error for partial deriv w.r.t. %s is %s."
            % (node.node_name, parent.node_name, max_rel_err)
        )
        overall_max_rel_err = max(max_rel_err, overall_max_rel_err)
    return overall_max_rel_err


def test_ComputationGraphFunction(
    graph, input_vals, outcome_vals, parameter_vals, delta=1e-7
):
    ...
    for param in parameter_vals:
        ...
        while not it.finished:
            ...
            partial_backprop = gradients[param][
                it.multi_index
            ]  # partial from ComputationGraphFunction
            rel_err = relative_error(partial_est, partial_backprop)
            max_rel_err = max(max_rel_err, rel_err)
            it.iternext()
        logging.debug(
            "(Parameter %s) Max rel error for partial deriv %s." % (param, max_rel_err)
        )
